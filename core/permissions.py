...
VISIBILITY_ACTIONS = {
    ...
    Content.PENDING: "content.pend",
    Content.PUBLISHED: "content.publish",
    Content.HIDDEN: "content.hide",
}
    ...
    if _type == "content":
        ...
        if action == "edit":
            return (
                ...
                or (
                    ...
                    or obj.visibility == Content.HIDDEN
                    and user.has_perm("hide_content")
                )
                and (
        ...
        if action == "pend":
            return (
                ...
                and user.has_perm("core.publish_content")
                or obj.visibility == Content.HIDDEN
                and user.has_perm("core.hide_content")
            )
        if action == "publish":
            return (
                obj.visibility == Content.PENDING
                or (
                    obj.visibility == Content.HIDDEN
                    and user.has_perm("code.hide_content")
                )
            ) and user.has_perm("core.publish_content")
        if action == "hide":
            return obj.visibility != Content.HIDDEN and user.has_perm(
                "core.hide_content"
            )
    if _type == "users":
        ...
        if action == "edit_profile":
            return (
                user.has_perm("auth.manage_users")
                or user == obj
                and user.has_perm("core.edit_profile")
            )
