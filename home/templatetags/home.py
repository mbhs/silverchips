...
def thumb(content, thumb_type=None):
    ...
    if (
        isinstance(content, models.Story) or isinstance(content, models.Video) or isinstance(content, models.Art)
    ) and content.cover:
...
def author_list(content):
    ...
    if content.guest_authors:
        name_list.append(content.guest_authors)
    if len(name_list) == 0:
        return "no one"
